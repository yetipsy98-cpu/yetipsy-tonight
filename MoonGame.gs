...
function setupMoonGame(){
  ...
  const staffAccounts=moon_ensureSheet_(ss,MOON_STAFF_ACCOUNTS,STAFF_ACCOUNT_HEADERS);

  moon_defaultConfig_(cfg,"ACTIVITY_ENABLED","TRUE","TRUE=开放 / FALSE=关闭");
  /* V13 改用 MOON_STAFF_ACCOUNTS 管理多个员工；保留旧 config 不再作为主要登入来源。 */
  moon_defaultConfig_(cfg,"ADMIN_PASSWORD","CHANGE-ADMIN","Admin 页面密码");
  moon_defaultConfig_(cfg,"GAME_TOKEN_TTL_MINUTES","20","二维码游戏有效分钟");
  moon_defaultConfig_(cfg,"STAFF_LOGIN_DAYS","30","员工设备保持登入天数");
  moon_defaultConfig_(cfg,"WHATSAPP_SEND_DAYS","3","奖励承诺发送工作日");

  if(staffAccounts.getLastRow()<2)staffAccounts.getRange(2,1,3,5).setValues([
...
function moon_gameTtl_(ss){return Math.max(2,Math.min(60,Number(moon_config_(ss,"GAME_TOKEN_TTL_MINUTES","20"))||10))*60000}
...
function moon_staffAccount_(ss,username){
  const sh=ss.getSheetByName(MOON_STAFF_ACCOUNTS);if(!sh||sh.getLastRow()<2)return null;
  const target=String(username||"").trim().toLowerCase();
  const vals=sh.getRange(2,1,sh.getLastRow()-1,5).getValues();
  for(let i=0;i<vals.length;i++){
    if(String(vals[i][0]).trim().toLowerCase()===target){
      return{row:i+2,username:String(vals[i][0]).trim(),password:String(vals[i][1]),displayName:String(vals[i][2]||vals[i][0]),enabled:String(vals[i][3]).toUpperCase()==="TRUE",authVersion:Number(vals[i][4])||1};
    }
  }
  return null;
}
function moon_staffLogin_(ss,d){
  ...
  const t=Utilities.getUuid(),now=new Date(),days=Math.max(1,Math.min(90,Number(moon_config_(ss,"STAFF_LOGIN_DAYS","30"))||30)),exp=new Date(now.getTime()+days*86400000);
  ss.getSheetByName(MOON_STAFF).appendRow([t,now,exp,a.username,a.authVersion]);
  const ttl=Math.max(1,Math.min(21600,Math.floor((exp.getTime()-Date.now())/1000)));
  CacheService.getScriptCache().put("moon_staff_"+t,JSON.stringify({exp:exp.getTime(),u:a.username,v:a.authVersion}),ttl);
  return{ok:true,staffToken:t,expiresAt:exp.toISOString(),username:a.username,displayName:a.displayName};
}
function moon_staffAuth_(ss,t){
  ...
  if(hit){try{const h=JSON.parse(hit),a=moon_staffAccount_(ss,h.u);if(h.exp>Date.now()&&a&&a.enabled&&a.authVersion===Number(h.v))return true}catch(e){}}
  const sh=ss.getSheetByName(MOON_STAFF);if(!sh||sh.getLastRow()<2)return false;
  const f=sh.getRange(2,1,sh.getLastRow()-1,1).createTextFinder(t).matchEntireCell(true).findNext();if(!f)return false;
  const row=f.getRow(),vals=sh.getRange(row,1,1,5).getValues()[0],exp=new Date(vals[2]).getTime(),u=String(vals[3]),v=Number(vals[4])||1,a=moon_staffAccount_(ss,u);
  if(!exp||exp<=Date.now()||!a||!a.enabled||a.authVersion!==v)return false;
  cache.put("moon_staff_"+t,JSON.stringify({exp,u,v}),Math.max(1,Math.min(21600,Math.floor((exp-Date.now())/1000))));
  return true;
}
...
function moon_validGame_(ss,t){
  ...
  // 兼容旧版生成的 UNLOCKED 与新版 READY。
  const status=String(s.data[4]||"").toUpperCase();
  const usedAt=s.data[3];
  if(usedAt || status==="USED")return{ok:false,error:"game_used"};
  ...
  // Apps Script / Sheets 日期有时会以 Date、数字或字符串返回，统一处理。
  let exp=s.data[2],expMs=0;
  if(exp instanceof Date)expMs=exp.getTime();
  else if(typeof exp==="number")expMs=exp;
  else expMs=new Date(exp).getTime();

  if(!expMs)return{ok:false,error:"invalid_game_expiry"};
  if(Date.now()>expMs){
    sh.getRange(s.row,5).setValue("EXPIRED");
    return{ok:false,error:"game_expired"};
  }
...
function moon_adminState_(ss,d){
  ...
  return{ok:true,config:{activityEnabled:moon_enabled_(ss),staffUsername:String(moon_config_(ss,"STAFF_USERNAME","staff")),gameTtl:Number(moon_config_(ss,"GAME_TOKEN_TTL_MINUTES","20")),staffHours:Number(moon_config_(ss,"STAFF_LOGIN_HOURS","12")),sendDays:Number(moon_config_(ss,"WHATSAPP_SEND_DAYS","3"))},rewards:moon_rewards_(ss),claims};
}
function moon_adminSaveConfig_(ss,d){
  ...
  if(d.staffDays)moon_setConfig_(ss,"STAFF_LOGIN_DAYS",String(Math.max(1,Math.min(90,Number(d.staffDays)||30))));
